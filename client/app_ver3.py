...
while True:
    ...
    closed_apps = {app: int(time.time()) for app in current_apps if app not in running_apps}
    ...
    for app, close_time in closed_apps.items():
        print(f"Closed App: {app}, Close Time: {close_time}")
    ...
    for app, timestamp in new_apps.items():
        date_ref.child('app_history').push().set({'app_name': app, 'start-time': timestamp})

    for app, close_time in closed_apps.items():
        date_ref.child('app_history').push().set({'app_name': app, 'close-time': close_time})
